...
This folder owns comparison workflows for evaluating farm, field, treatment, or time-window differences in EDA outputs.
...
Run `./scripts/validate.sh` from the repository root after structural changes. If the guide names a local comparison command, run that command against the smallest available sample when dependencies are available.
