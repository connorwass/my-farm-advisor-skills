...
This folder owns correlation workflows for checking relationships among field observations, weather, soil, imagery, and derived agronomic variables.
...
Run `./scripts/validate.sh` from the repository root after structural changes. If the guide names a local correlation command, run that command against the smallest available sample when dependencies are available.
