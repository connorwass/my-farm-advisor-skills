...
This folder owns visualization workflows for turning EDA results into clear charts and field-level visual summaries.
...
Run `./scripts/validate.sh` from the repository root after structural changes. If the guide names a local visualization command, run that command against the smallest available sample when dependencies are available.
