...
This folder owns headlands-ring workflows for field-edge and headland geometry analysis.
...
Run `./scripts/validate.sh` from the repository root after structural changes. If the guide names a local headlands command, run that command against the smallest available sample when dependencies are available.
